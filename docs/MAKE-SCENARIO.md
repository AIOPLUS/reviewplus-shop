...
| 3. Mollie-statusmelding | `type = status` | antwoord `200 ok` → record ophalen → betaling opvragen bij Mollie → bij `paid`: record bijwerken + mail "Betaling ontvangen" (bij `soort = preorder`: "Pre-order betaald" + bevestiging aan de klant, module 179) |
| 4. Klant terug van Mollie | `type = return` | record ophalen → betaling opvragen → 302 naar `/aanvragen?status=betaald` (paid/authorized/pending) of `?status=geannuleerd`; bij `soort = preorder` naar `/live-reviewteller?status=…#preorder` |
...
| 5b. Contactbericht (reviewplus.io) | `request_type = contact` én Turnstile ok | antwoord `{"ok": true}` → mail "Contactbericht via reviewplus.io" aan jou (Reply-To = afzender, dus direct beantwoorden) → kopie van het bericht aan de afzender. Route 2 slaat contactberichten over. Ook de offerteaanvragen voor de live reviewteller uit de shop lopen via deze route (`lead_source: shop-reviewteller`; platform, cijfers en aantal staan in het bericht en in het veld `offerte`), zie `docs/REVIEWTELLER.md`. |
| 5c. Sollicitatie (reviewplus.io/jobs) | `request_type = sollicitatie` én Turnstile ok | antwoord `{"ok": true}` → mail "Sollicitatie <vacature>: <naam>" aan jou (Reply-To = kandidaat) met telefoon, woonplaats, LinkedIn/cv-link en motivatie → bevestiging aan de kandidaat (cv als bijlage terugsturen kan via antwoorden). Route 2 slaat sollicitaties over. Bewaartermijn: 4 weken na de procedure, met toestemming max. 1 jaar. |
| 5d. Abonnement afgesloten (reviewplus.io/aanmelden) | `request_type = abonnement`, Turnstile ok, geldig plan (`1-jaar`/`2-jaar`/`3-jaar`, `maand`/`jaar`) en beide akkoordvinkjes | antwoord `{"ok": true}` → prijs opnieuw berekenen (module 121, gelijk houden met `content.ts` van de site) → Teamleader: bedrijf/contact zoeken of aanmaken (tag `review-plus-klant`), deal in fase **Offerte Getekend** met contractwaarde, taken "Eerste factuur versturen" en "Welkomstpakket verzenden" → account op app.reviewplus.io via EmbedMyReviews Agency API (`config:emr_token` + `config:emr_plan` in de datastore; zonder die records wordt deze stap overgeslagen) → mail aan jou (met accountstatus) → welkomstmail aan de klant met het vastgelegde akkoord. Btw (module 144): 21%, of verlegd bij een Belgisch bedrijf met geldig btw-nummer; bedragen per factuur excl./btw/incl. in deal, taak en mails. Route 2 slaat abonnementen over. |
| 5e. Pre-order live reviewteller (shop) | `request_type = preorder` én Turnstile ok (modules 170–178, sinds 26-09-2026) | `products.json` ophalen → bedrag zelf berekenen (prijs uit `offerte_producten` × aantal 1–10, + 21% btw of verlegd voor BE) → bij geldige invoer: Mollie-betaling (`testmode` in module 171 = `false`: live sinds 26-09-2026) → record `betaling:<ref>` met `soort: preorder` en in `waarde` de bestelling en de btw-tekst (`omschrijving|btw`) → antwoord `{"ok": true, "checkoutUrl": …}` → mail "Pre-order reviewteller (nog niet betaald)" aan jou. Ongeldig → `{"ok": false}` (400), geen betaling. Route 2 slaat pre-orders over. Zie `docs/REVIEWTELLER.md`. |
...
| `betaling:<lead_ref>` | `payment_id`, `bedrag`, `status`, `bedrijf`, `email`, `testmode`, `aangemaakt`, `soort` (`betaling` = extra's, `preorder` = reviewteller; bij een pre-order ook `bedrijfsnummer` en de bestelling in `waarde`) |
