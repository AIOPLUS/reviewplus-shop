...
| 5b. Contactbericht (reviewplus.io) | `request_type = contact` én Turnstile ok | antwoord `{"ok": true}` → mail "Contactbericht via reviewplus.io" aan jou (Reply-To = afzender, dus direct beantwoorden) → kopie van het bericht aan de afzender. Route 2 slaat contactberichten over. |
